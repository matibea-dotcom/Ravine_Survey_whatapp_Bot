...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    for (const raw of messages) {
      ...
      // Meta rolled out a business-scoped user_id (BSUID) identity field in
      // 2026. When a person has hidden their phone number, `from`/`wa_id`
      // can be OMITTED entirely from the webhook, leaving only user_id. Fall
      // back through every identifier we might have rather than silently
      // passing `undefined` as the recipient (which Meta's send API
      // rejects with "The parameter to is required" — no reply ever goes
      // out, and the person is left hanging with zero response).
      const bsuid = raw.from_user_id || value.contacts?.[0]?.user_id || null;
      const waId = raw.from || value.contacts?.[0]?.wa_id || bsuid;

      if (!waId) {
        console.error("No usable sender identifier on this message — skipping. Raw:", JSON.stringify(raw));
        continue;
      }
      if (!raw.from) {
        console.warn(
          "Message has no phone-number identifier, using fallback:",
          waId,
          "— reply delivery to this identifier is unverified; see BSUID note."
        );
      }
      ...
      const replies = await engine.handleInboundMessage(waId, normalized, bsuid);
...
// Startup is async because agentStore is now Sheets-backed (ensureStorageReady
// makes a real API call to confirm the Agents tab is reachable). Awaiting
// this before app.listen() means a bad service-account/sheet-ID config fails
// loudly at deploy time instead of surfacing later on someone's first message.
(async () => {
  try {
    await agentStore.ensureStorageReady();
    console.log("✅ Agent storage (Google Sheets) initialized successfully");
  } catch (err) {
