...
over WhatsApp (Meta Cloud API), across three survey tracks (General Trade,
Modern Trade, Insurance — placeholder). Registrations, store history, and
submissions are all stored in Google Sheets so data survives Render
redeploys (Render's free tier has no persistent disk).
...
- **Per-track survey flow**: each track's questions live in its own file
  under `src/surveys/` (`gt.js`, `mt.js`, `insurance.js`),
  ...
  - **Insurance**: placeholder track, real questions to follow.
- **HQ/Warehouse visits**: not a separate track — an MT agent's day mixes
  mostly retail stops with the occasional visit to the warehouse/HQ itself
  (sorting stocking issues, payments, and branch escalations, not selling).
  Picking "Warehouse/HQ" as the store type on a new visit swaps the entire
  retail question set for five focused HQ questions (Ravine stock + W/S
  pricing, competitor W/S + retail pricing, statement/invoice/payment
  issues, delivery issues, AOB), and routes the submission to its own
  `Warehouse_Submissions` tab — kept separate from `MT_Submissions` since
  it's a different kind of data (a discussion log, not a shelf audit), not
  merged into 200+ mostly-blank retail columns. Same agent, same MT
  registration, no SWITCHTRACK needed.
...
- **Google Sheets storage**: `Agents`, `Stores`, one Submissions tab per
  ...
  `Insurance_Submissions`), plus `Warehouse_Submissions` (HQ/Warehouse
  visits — a storeType branch within MT, see above, not its own track),
...
1. Create `src/surveys/<key>.js` matching the shape of `gt.js`/`mt.js`
   (export `label`, `sheetTabEnvVar`, `defaultSheetTab`,
...
src/
  ...
  surveys/
    ...
    mt.js             Modern Trade survey questions (the primary track — includes
                       the HQ/Warehouse storeType branch, see "What's implemented")
