...
const { parseOtherCategoryPricing } = require("./otherCategoryPriceParser");
const {
  REGISTRATION_STEPS,
  TRACK_ORDER,
  trackLabel,
...
const ADMIN_COMMANDS = ["REPORT", "PRICING", "COLLECTIONS", "RECONCILE", "TOPBRANDS", "MYDATA", "STATS"];
...
// A step tagged retailOnly: true is part of the normal shelf/merchandising
// survey and doesn't apply when the visit itself is to the warehouse/HQ
// rather than a retail outlet (see storeType === "Warehouse/HQ" in mt.js).
// Checked here, once, rather than adding this condition to every retail
// step's own skipIf individually.
function activeSteps(session) {
  const isHqVisit = session.answers.storeType === "Warehouse/HQ";
  return surveyStepsFor(session).filter((s) => {
    if (s.skipIf && s.skipIf(session.answers)) return false;
    if (s.retailOnly && isHqVisit) return false;
    if (s.hqOnly && !isHqVisit) return false;
    return true;
  });
}
...
function advance(session, answeredKey) {
  if (answeredKey) {
    ...
    if (rawIdx !== -1) {
      const isHqVisit = session.answers.storeType === "Warehouse/HQ";
      const activeCount = allSteps.slice(0, rawIdx + 1).filter((s) => {
        if (s.skipIf && s.skipIf(session.answers)) return false;
        if (s.retailOnly && isHqVisit) return false;
        if (s.hqOnly && !isHqVisit) return false;
        return true;
...
// Shared by the plain single-field edit path and by loop completions that
// were triggered from an edit: re-lands on the first still-unanswered
// REQUIRED active step (never an optional gap — see the advance() fix
// this mirrors) so a stray follow-up message can't land on, and silently
// overwrite, the wrong field.
function finishEditPosition(session) {
...
function promptForRavinePriceLoopOrContinue(session) {
  ...
  if (loop.index >= loop.skus.length) {
    const wasEdit = loop.isEdit;
    session.ravinePriceLoop = null;
    if (wasEdit) {
      finishEditPosition(session);
      return ["✅ Updated *SKUs Stocked* (pricing/facings for the new selection captured).\n\nType SUMMARY to review, or SUBMIT if you're done."];
    }
    advance(session, loop.stepKey);
    return promptForCurrentOrSummary(session);
  }
...
function startCompetitorLoop(session, brands, stepKey, isEdit) {
  session.competitorLoop = { brands, brandIndex: 0, stage: "categories", categoryIndex: 0, stepKey, isEdit: !!isEdit };
}
...
function promptForCompetitorLoopOrContinue(session) {
  ...
  if (loop.brandIndex >= loop.brands.length) {
    ...
    if (wasEdit) {
      finishEditPosition(session);
      return ["✅ Updated *Competitor Brands* (pricing for the new selection captured).\n\nType SUMMARY to review, or SUBMIT if you're done."];
    }
    advance(session, loop.stepKey);
    return promptForCurrentOrSummary(session);
  }
  ...
  if (loop.stage === "categoryPricing") {
    ...
    return [
      `What *sizes/packs* does *${brand}* have in *${cat}*, and at what price?\n` +
        `Put each one on its own line, like this:\n` +
        `500ml - 65 - 60\n` +
        `(that's: size - regular price - promo price)\n` +
        `No current promo on a size? Just leave that part off: 500ml - 65\n` +
        `Don't have this info? Reply SKIP.`,
    ];
  }
  return [""]; // unreachable
}
...
function handleCompetitorLoop(session, message, upper, replies) {
  ...
  if (loop.stage === "regularPrice") {
    // A generous ceiling (well above anything in the real catalog, which
    // tops out around 4,000 for bulk items) that still catches obvious
    // mis-entries -- "Reg 50061, Promo 200500500" was a real submission,
    // almost certainly a typo, that the old unbounded validator accepted.
    const result = validators.validateNumeric(getRawText(message), { allowZero: false, max: 20000 });
    if (!result.ok) {
      replies.push(result.error);
      return replies;
    }
  ...
  if (loop.stage === "categoryPricing") {
    ...
    if (upper === "SKIP") {
      ...
      // Generous limit — this field is meant to hold several products'
      // worth of pricing, "one per line", not a brief comment. The
      // default 500-char cap was silently rejecting/truncating agents
      // trying to list a full category's competitor lineup.
      const result = validators.validateComments(getRawText(message), { max: 2000 });
      if (!result.ok) {
        replies.push(result.error);
        return replies;
      }
...
function processAnswer(step, message, answers) {
  ...
  switch (step.type) {
    ...
    case "comments":
      return validators.validateComments(raw, step.opts);
    case "phone":
...
async function handleAdminCommand(cmd, replies) {
  switch (cmd) {
    ...
    case "TOPBRANDS": {
      try {
        const stats = await sheets.computeTopBrandsReport();
        if (!stats.totalVisits) {
          replies.push("No MT submissions yet to report on.");
          return replies;
        }
        const lines = [
          "🏷️ *Top Brands by Mention Frequency*",
          `From ${stats.totalVisits} MT visits`,
          "",
          "*Current fixed list — how they're actually performing:*",
        ];
        for (const [brand, count] of stats.fixedRanked) {
          lines.push(`${brand}: ${count} mention(s)`);
        }
        lines.push("", "*Brands outside the fixed list (worth reviewing for inclusion):*");
        if (stats.otherRanked.length === 0) {
          lines.push("None recorded yet.");
        } else {
          for (const [brand, count] of stats.otherRanked.slice(0, 15)) {
            lines.push(`${brand}: ${count} mention(s)`);
          }
          if (stats.otherRanked.length > 15) {
            lines.push(`...and ${stats.otherRanked.length - 15} more`);
          }
        }
        lines.push("", "This list is for your review — the fixed 7 in the survey only update when you edit COMPETITOR_BRANDS in mt.js.");
        replies.push(lines.join("\n"));
      } catch (err) {
        console.error("TOPBRANDS failed:", err.message);
        replies.push("⚠️ Couldn't generate the top-brands report right now. Try again shortly.");
      }
      return replies;
    }
    case "MYDATA":
...
async function handleSubmit(waId, agent, session, replies) {
  const steps = activeSteps(session);
  const missing = steps.filter((s) => s.required && session.answers[s.key] === undefined);
  if (missing.length > 0) {
    replies.push(
...
async function finalizeSubmit(waId, agent, session, replies) {
  ...
  try {
          ...
          for (const [category, rawText] of Object.entries(byCategory)) {
            ...
            for (const entry of parsed) {
              detailRows.push({
                ...
                brand,
                category,
                packagingType: entry.packagingType || "Unspecified",
                variant: entry.variant,
                stockStatus: entry.stockOut ? "Out of Stock" : "",
    ...
    // Best-effort: parse the free-text "other dairy categories" note (e.g.
    // milk powder pricing across brands not in the fixed catalog) into
    // structured rows too — same reasoning as above, never blocks submission.
    if (session.track === "MT" && session.answers.otherDairyCategoriesNote) {
      try {
        const parsed = parseOtherCategoryPricing(session.answers.otherDairyCategoriesNote);
        if (parsed.length > 0) {
          ...
          await sheets.appendOtherCategoryPriceDetails(rows);
        }
      } catch (err) {
        console.error("Other-category price detail parsing/write failed (non-blocking):", err.message);
      }
    }
  } catch (err) {
...
async function handleInboundMessage(waId, message) {
  ...
  if (targetKey) {
    ...
    // Editing these two fields changes which follow-up questions are
    // required (per-SKU pricing/facings, per-brand pricing) — re-running
    // the loop for the NEW selection, rather than just storing the new
    // list and calling it done, is what actually keeps the submission's
    // supporting data consistent with what was just edited. This mirrors
    // the non-edit trigger logic further below, just flagged isEdit so the
    // loop lands back in "post-edit" position instead of advancing forward.
    if (step.key === "skusStocked" && Array.isArray(result.value) && result.value.length > 0) {
      const keep = new Set(result.value);
      if (session.answers.ravineSkuPricing) {
        for (const sku of Object.keys(session.answers.ravineSkuPricing)) {
          if (!keep.has(sku)) delete session.answers.ravineSkuPricing[sku];
        }
      }
      if (session.answers.ravineSkuDetails) {
        for (const sku of Object.keys(session.answers.ravineSkuDetails)) {
          if (!keep.has(sku)) delete session.answers.ravineSkuDetails[sku];
        }
      }
      // Only ask about SKUs that don't already have pricing captured —
      // confirmed/prefilled data from a revisit (or from earlier in this
      // same edit) shouldn't have to be re-typed just because one new SKU
      // got added to the selection.
      const newSkus = result.value.filter((sku) => !session.answers.ravineSkuPricing || !session.answers.ravineSkuPricing[sku]);
      if (newSkus.length === 0) {
        finishEditPosition(session);
        replies.push(`✅ Updated *SKUs Stocked*.`);
        replies.push("Type SUMMARY to review, or SUBMIT if you're done.");
        return replies;
      }
      session.ravinePriceLoop = { skus: newSkus, index: 0, stage: "price", stepKey: step.key, isEdit: true };
      replies.push(`Updating *SKUs Stocked* — let's capture pricing/facings for the ${newSkus.length} new SKU${newSkus.length > 1 ? "s" : ""}.`);
      replies.push(...promptForRavinePriceLoopOrContinue(session));
      return replies;
    }
    if (
      ...
      const keep = new Set(result.value);
      for (const store of [session.answers.competitorPricing, session.answers.competitorCategories, session.answers.competitorCategoryPricing]) {
        if (!store) continue;
        for (const brand of Object.keys(store)) {
          if (!keep.has(brand)) delete store[brand];
        }
      }
      // Same principle: only walk through pricing for brands that aren't
      // already captured, not the whole ranked list every time.
      const newBrands = result.value.filter((b) => !session.answers.competitorPricing || !session.answers.competitorPricing[b]);
      if (newBrands.length === 0) {
        finishEditPosition(session);
        replies.push(`✅ Updated *Competitor Brands*.`);
        replies.push("Type SUMMARY to review, or SUBMIT if you're done.");
        return replies;
      }
      startCompetitorLoop(session, newBrands, step.key, true);
      replies.push(`Updating *Competitor Brands* — let's capture pricing for the ${newBrands.length} new brand${newBrands.length > 1 ? "s" : ""}.`);
      replies.push(...promptForCompetitorLoopOrContinue(session));
      return replies;
    }
    if (step.key === "competitorBrandsRanked" && Array.isArray(result.value) && result.value.includes("Other")) {
      // "Other" needs a name before pricing can start — chain the edit
      // forward into that field instead of dropping it, same as the
      // non-edit path does.
      session.editingField = "competitorOtherBrandNames";
      replies.push(`✅ Updated *${step.label}*.`);
      replies.push("You selected 'Other' — name the additional brand(s), comma separated.");
      return replies;
    }
    if (step.key === "competitorOtherBrandNames") {
      ...
      const newBrands = expanded.filter((b) => !session.answers.competitorPricing || !session.answers.competitorPricing[b]);
      if (newBrands.length === 0) {
        finishEditPosition(session);
        replies.push(`✅ Updated *Other Competitor Brand Names*.`);
        replies.push("Type SUMMARY to review, or SUBMIT if you're done.");
        return replies;
      }
      startCompetitorLoop(session, newBrands, step.key, true);
