...
async function handleInboundMessage(waId, message) {
  ...
  if (targetKey) {
    ...
    if (step.key === "skusStocked" && Array.isArray(result.value) && result.value.length > 0) {
      ...
      session.ravinePriceLoop = { skus: result.value, index: 0, stage: "price", stepKey: step.key, isEdit: true };
      replies.push(`Updating *SKUs Stocked* — let's go through pricing/facings for the new selection.`);
      replies.push(...promptForRavinePriceLoopOrContinue(session));
      return replies;
    }
    if (
      ...
      startCompetitorLoop(session, result.value, step.key, true);
      replies.push(`Updating *Competitor Brands* — let's go through pricing for the new selection.`);
      replies.push(...promptForCompetitorLoopOrContinue(session));
      return replies;
    }
    ...
    if (step.key === "competitorOtherBrandNames") {
      ...
      startCompetitorLoop(session, expanded, step.key, true);
      replies.push(`Updating *Competitor Brands* — let's go through pricing for the new selection.`);
      replies.push(...promptForCompetitorLoopOrContinue(session));
      return replies;
    }
