...
async function appendCompetitorPriceDetails(rows) {
  ...
  const values = rows.map((r) =>
    COMPETITOR_PRICE_DETAIL_COLUMNS.map((c) => (c === "submittedAtDate" ? toSheetDateTime(r.submittedAt) : r[c] ?? ""))
  );
  await sheetsApi.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
...
async function appendOtherCategoryPriceDetails(rows) {
  if (!rows || rows.length === 0) return;
  await ensureHeaderHasColumns(OTHER_CATEGORY_PRICE_DETAIL_TAB, OTHER_CATEGORY_PRICE_DETAIL_COLUMNS);
  const sheetsApi = await getClient();
  const values = rows.map((r) =>
    OTHER_CATEGORY_PRICE_DETAIL_COLUMNS.map((c) => (c === "submittedAtDate" ? toSheetDateTime(r.submittedAt) : r[c] ?? ""))
  );
  await sheetsApi.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
