...
// Sheets does not recognize a raw .toISOString() string (the "T"/"Z"
// separators aren't part of its date auto-detection) — so every timestamp
// column has actually been landing as plain, ungroupable TEXT, which is
// exactly what breaks pivot-table "group by week/month". This produces a
// companion value Sheets DOES parse as a real date-time, written as an
// ADDITIONAL column alongside the existing ISO string rather than
// replacing it — nothing that already reads the ISO field for date-math
// (RECONCILE, COLLECTIONS, getLatestSubmissionForStore, etc.) is touched.
function toSheetDateTime(isoString) {
  if (!isoString) return "";
...
// Caches which tabs are known to exist so ensureTabExists doesn't hit
// spreadsheets.get() (a full, relatively expensive "list all tabs" call)
// on every single write. A submission that touches 3 tabs previously cost
// 6 of these calls (ensureHeaderHasColumns called ensureTabExists directly
// AND again via readHeader); this collapses it to at most 1 per tab per
// server lifetime, since a tab practically never gets deleted mid-session.
 ...
 * instead of crashing the submission. Cached after the first successful
 * check/creation — see knownTabs above.
 */
...
function flattenSubmission(submission, columns) {
  ...
  // One-hot presence flags (1/0) for every fixed-vocabulary multiselect
  // field — comma-joined text columns like competitorBrandsRanked are
  // accurate but painful to query (need SEARCH/regex-style formulas even
  // for a simple "how many visits had Brookside present" count). These
  // give every fixed value its own column so COUNTIF/SUMIF/pivot tables
  // work directly, no text parsing needed. Kept alongside the original
  // comma-joined column, not replacing it — that stays as the
  // human-readable reference.
  const CATEGORY_ONEHOT_PREFIX = "Category 1/0: ";
  const SKU_ONEHOT_PREFIX = "SKU 1/0: ";
  const COMPETITOR_ONEHOT_PREFIX = "Competitor 1/0: ";
  const POS_ONEHOT_PREFIX = "POS 1/0: ";
  const categoriesStocked = a.ravineCategoriesStocked || [];
  const skusStockedList = a.skusStocked || [];
  const competitorBrandsRankedList = a.competitorBrandsRanked || [];
  const posMaterialsList = a.posMaterialsPresent || [];

  return columns.map((col) => {
    switch (col) {
      case "referenceNumber": return submission.referenceNumber;
      case "submittedAt": return submission.submittedAt;
      case "submittedAtDate": return toSheetDateTime(submission.submittedAt);
      ...
      default: {
        if (col.startsWith(CATEGORY_ONEHOT_PREFIX)) {
          const cat = col.slice(CATEGORY_ONEHOT_PREFIX.length);
          return categoriesStocked.includes(cat) ? 1 : 0;
        }
        if (col.startsWith(SKU_ONEHOT_PREFIX)) {
          const sku = col.slice(SKU_ONEHOT_PREFIX.length);
          return skusStockedList.includes(sku) ? 1 : 0;
        }
        if (col.startsWith(COMPETITOR_ONEHOT_PREFIX)) {
          const brand = col.slice(COMPETITOR_ONEHOT_PREFIX.length);
          return competitorBrandsRankedList.includes(brand) ? 1 : 0;
        }
        if (col.startsWith(POS_ONEHOT_PREFIX)) {
          const material = col.slice(POS_ONEHOT_PREFIX.length);
          return posMaterialsList.includes(material) ? 1 : 0;
        }
...
async function appendSubmission(submission) {
  ...
  const isHqVisit = track === "MT" && submission.answers.storeType === "Warehouse/HQ";
  const sheetTab = isHqVisit ? HQ_SUBMISSIONS_TAB : getSheetTabForTrack(track);
  const columns = isHqVisit ? HQ_COLUMNS : getColumnsForTrack(track);

  // Use the ACTUAL header order back from ensureHeaderHasColumns, not the
  // raw code `columns` list. A sheet that already has a header only ever
  // gets new columns appended at the end (see ensureHeaderHasColumns) — it
  // is never reordered to match the code. Building the row from `columns`
  // instead of this returned order silently misaligns every write once
  // the two diverge, which happens any time the code's column order
  // changes after the sheet's header already exists.
  const header = await ensureHeaderHasColumns(sheetTab, columns);
  const sheetsApi = await getClient();
  await sheetsApi.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${sheetTab}!A1`,
    valueInputOption: "USER_ENTERED",
    insertDataOption: "INSERT_ROWS",
    requestBody: { values: [flattenSubmission(submission, header)] },
  });
}
...
const COMPETITOR_PRICE_DETAIL_COLUMNS = [
  "referenceNumber", "submittedAt", "submittedAtDate", "storeId", "accountName", "brand",
  "category", "packagingType", "variant", "stockStatus", "stockoutReason", "regularPrice", "promoPrice", "parsed", "rawLine",
];
...
async function appendCompetitorPriceDetails(rows) {
  ...
  const header = await ensureHeaderHasColumns(COMPETITOR_PRICE_DETAIL_TAB, COMPETITOR_PRICE_DETAIL_COLUMNS);
  const sheetsApi = await getClient();
  const values = rows.map((r) =>
    header.map((c) => (c === "submittedAtDate" ? toSheetDateTime(r.submittedAt) : r[c] ?? ""))
  );
  await sheetsApi.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
...
// HQ/Warehouse visits are a storeType branch within MT (not a track — see
// mt.js), but the data itself is different enough (a discussion log, not
// a shelf audit) that it goes to its own tab rather than getting merged
// into MT_Submissions' 200+ retail columns, where it would leave almost
// every column blank on both sides.
const HQ_SUBMISSIONS_TAB = process.env.GOOGLE_SHEET_TAB_WAREHOUSE || "Warehouse_Submissions";
const HQ_COLUMNS = [
  "referenceNumber", "submittedAt", "submittedAtDate", "sessionId",
  "agentWaId", "agentFullName", "agentId", "agentRegion", "agentCompany",
  "storeId", "accountName", "areaLocation",
  "hqRavineStockWsPricing", "hqCompetitorPricing",
  "hqPaymentIssues", "hqDeliveryIssues", "hqAob", "flags",
];

const OTHER_CATEGORY_PRICE_DETAIL_TAB = process.env.GOOGLE_SHEET_TAB_OTHER_CATEGORY_PRICE_DETAIL || "OtherCategoryPriceDetail";
const OTHER_CATEGORY_PRICE_DETAIL_COLUMNS = [
  "referenceNumber", "submittedAt", "submittedAtDate", "storeId", "accountName",
  "productCategory", "packagingType", "brand", "size", "price", "rawLine",
];

// Long-format table: one row per brand/size/price mention parsed out of the
// free-text "other dairy categories" note (categories outside Ravine's own
// catalog, e.g. milk powder — see otherCategoryPriceParser.js). A brand
// that's never been seen before just shows up as new rows here; no column
// or schema change needed. Pivot on brand (or productCategory) for a
// spreadsheet-side "column per brand" view.
async function appendOtherCategoryPriceDetails(rows) {
  if (!rows || rows.length === 0) return;
  const header = await ensureHeaderHasColumns(OTHER_CATEGORY_PRICE_DETAIL_TAB, OTHER_CATEGORY_PRICE_DETAIL_COLUMNS);
  const sheetsApi = await getClient();
  const values = rows.map((r) =>
    header.map((c) => (c === "submittedAtDate" ? toSheetDateTime(r.submittedAt) : r[c] ?? ""))
  );
  await sheetsApi.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${OTHER_CATEGORY_PRICE_DETAIL_TAB}!A1`,
    valueInputOption: "USER_ENTERED",
    insertDataOption: "INSERT_ROWS",
    requestBody: { values },
  });
...
function unflattenMtSubmission(record) {
  ...
  // "Other" brands (anything outside the fixed 7) were previously lost on
  // revisit prefill entirely — they're written to the catch-all column at
  // submit time but were never read back. Parses the same format
  // flattenSubmission generates: "Brand [Cat1/Cat2]: Reg 50, Promo -".
  // Note: per-category free-text detail for these brands isn't
  // recoverable here (only stored long-format in CompetitorPriceDetail,
  // not as its own MT_Submissions column) — regular/promo price and
  // category list are, which is the data that was actually going missing.
  const otherBrandsText = record["Other Competitor Brands & Pricing"];
  if (otherBrandsText) {
    for (const entry of otherBrandsText.split("; ")) {
      const m = entry.match(/^(.+?)\s*\[(.*?)\]:\s*Reg\s*(-|\d+(?:\.\d+)?),\s*Promo\s*(-|\d+(?:\.\d+)?)\s*$/);
      if (!m) continue;
      const [, brand, catsRaw, regular, promo] = m;
      if (!a.competitorBrandsRanked) a.competitorBrandsRanked = [];
      if (!a.competitorBrandsRanked.includes(brand)) a.competitorBrandsRanked.push(brand);
      a.competitorCategories = a.competitorCategories || {};
      a.competitorPricing = a.competitorPricing || {};
      if (catsRaw) a.competitorCategories[brand] = catsRaw.split("/").filter(Boolean);
      a.competitorPricing[brand] = {
        regular: regular !== "-" ? Number(regular) : null,
        promo: promo !== "-" ? Number(promo) : null,
      };
    }
...
// ---- Top brands by real mention frequency (informs periodic manual
// review of the fixed COMPETITOR_BRANDS list — see mt.js) ----
// Pulls from three sources:
//  1. competitorBrandsRanked — how often each of the CURRENT fixed 7
//     actually gets ranked, so you can see which are pulling their weight.
//  2. "Other Competitor Brands & Pricing" — the catch-all column, which
//     (unlike the raw competitorBrandsRanked field) holds the REAL typed
//     name once "Other" was picked and expanded, not the literal word
//     "Other". This is the main signal for brands outside the fixed list.
//  3. OtherCategoryPriceDetail's brand column — covers categories outside
//     Ravine's own catalog entirely (e.g. milk powder), same brands can
//     show up here too.
async function computeTopBrandsReport() {
  const mtTab = getSheetTabForTrack("MT");
  const { records: mtRecords } = await readAllRows(mtTab);
  if (mtRecords.length === 0) return { totalVisits: 0 };

  const fixedBrandCounts = {};
  for (const b of COMPETITOR_BRANDS) fixedBrandCounts[b] = 0;
  ...
  for (const r of mtRecords) {
    const ranked = (r.competitorBrandsRanked || "").split(",").map((s) => s.trim()).filter(Boolean);
    for (const b of ranked) {
      if (COMPETITOR_BRANDS.includes(b)) bump(fixedBrandCounts, b);
      // literal "Other" (unexpanded) is not a real brand name — skip it,
      // the catch-all column below has the real name instead.
    }

    const catchAll = r["Other Competitor Brands & Pricing"] || "";
    if (catchAll) {
      for (const entry of catchAll.split("; ")) {
        const bracketIdx = entry.indexOf(" [");
        const name = bracketIdx === -1 ? entry : entry.slice(0, bracketIdx);
        bump(otherBrandCounts, name);
      }
    }
  }

  try {
    const { records: otherCatRecords } = await readAllRows(OTHER_CATEGORY_PRICE_DETAIL_TAB);
    for (const r of otherCatRecords) {
      if (r.brand) bump(otherBrandCounts, r.brand);
    }
  } catch (err) {
    console.error("computeTopBrandsReport: could not read OtherCategoryPriceDetail:", err.message);
  }

  const fixedRanked = Object.entries(fixedBrandCounts).sort((a, b) => b[1] - a[1]);
  const otherRanked = Object.entries(otherBrandCounts).sort((a, b) => b[1] - a[1]);

  return { totalVisits: mtRecords.length, fixedRanked, otherRanked };
}
...
module.exports = {
  ...
  appendOtherCategoryPriceDetails,
  readAllStores,
  appendStore,
  computeMtReport,
