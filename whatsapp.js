...
function logAxiosError(context, err) {
  if (err.response) {
    console.error(
      `${context}: HTTP ${err.response.status}`,
      JSON.stringify(err.response.data)
    );
  } else if (err.request) {
    console.error(`${context}: no response received`, err.message);
  } else {
...
async function sendText(to, text) {
  ...
  for (const part of parts) {
    try {
      ...
      logAxiosError("Error sending WhatsApp message", err);
...
async function markRead(messageId) {
  try {
    ...
    logAxiosError("Warning: Failed to mark message as read", err);
  }
}
