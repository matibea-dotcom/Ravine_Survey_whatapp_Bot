 ...
 */
async function handleInboundMessage(waId, message, bsuid) {
  const replies = [];
  ...
  // Tolerant of either identifier type — see agentStore.js. If this message
  // carried a BSUID we haven't seen on file for this agent yet (most often:
  // their phone number is now hidden, or Meta sent both alongside each
  // other), remember it so a future phone-number-less message from the same
  // person still resolves to their existing registration instead of
  // treating them as unregistered.
  let agent = await agentStore.getAgentByAnyId(waId, bsuid);
  if (agent && bsuid) {
    agent = await agentStore.reconcileIdentifiers(agent, waId, bsuid);
  }
