...
function submissionDupeKey(session) {
  // GT dedupes on retailerName, MT on accountName — fall back to the first
  // text-ish answer if a track adds a differently-named identifying field.
  return session.answers.retailerName || session.answers.accountName || "";
}

function recordSubmissionForDupeCheck(waId, key) {
  const list = recentSubmissions.get(waId) || [];
  list.push({ key: key.toLowerCase(), at: Date.now() });
  recentSubmissions.set(waId, list.filter((e) => Date.now() - e.at < 24 * 3600 * 1000));
}

function findRecentDuplicate(waId, key) {
  if (!key) return null;
  const list = recentSubmissions.get(waId) || [];
  return list.find(
    (e) => e.key === key.toLowerCase() && Date.now() - e.at < 4 * 3600 * 1000
  );
}
...
async function handleInboundMessage(waId, message) {
  ...
  if (upper === "START") {
    ...
    replies.push(
      `New ${trackLabel(session.track)} survey started (Ref: ${session.sessionId}). Type HELP any time for commands.\n\n` +
  ...
  // Sessions created before this update (or restored without a track) default to GT.
  ...
  // Cross-field validation (e.g. RRP vs wholesale, overdue payment status, SOW 1.6/2.9)
  let flagNote = null;
...
function handleGlobalCommand(waId, agent, session, cmd, replies) {
  switch (cmd) {
    ...
    case "RESUME": {
      ...
      const step = currentStep(session);
      replies.push(
        `Resuming ${trackLabel(session.track)} survey (Ref: ${session.sessionId}).\n\n${stepPrompt(step, session.answers)}\n\n${progressLine(session)}`
      );
      return;
    }
    case "CANCEL":
...
async function handleSubmit(waId, agent, session, replies) {
  ...
  // Duplicate detection (SOW 2.4/2.9): same store/account, same agent, within 4h.
  const dupeKey = submissionDupeKey(session);
...
async function finalizeSubmit(waId, agent, session, replies) {
  ...
  recordSubmissionForDupeCheck(waId, submissionDupeKey(session));
...
function handleRegistration(waId, message, replies) {
  ...
  if (regState.index >= REGISTRATION_STEPS.length) {
    const profile = { ...regState.answers, surveyTrack: regState.answers.surveyTrack };
    agentStore.registerAgent(waId, profile);
  ...
  const nextStep = REGISTRATION_STEPS[regState.index];
  replies.push(nextStep.prompt);
  return replies;
}
